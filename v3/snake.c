...
bool progresser(int positionsX[TAILLE], int positionsY[TAILLE], char direction,char plateau[HAUTEUR][LARGEUR]);
void effacerEcran();
void disableEcho();
void enableEcho();
...
int main(){
	...
	while(peutAvancer){ 
		...
		peutAvancer = progresser(posX, posY,direction,plateau);
...
void initPaves(char plateau[HAUTEUR][LARGEUR]){
	...
	for (int pave_place = 0; pave_place < NB_PAVES; pave_place++) {
		...
		while ((x > (INIT_X - TAILLE - TAILLE_PAVES) && x < INIT_X + TAILLE_PAVES) || (y > INIT_Y + TAILLE_PAVES && y < INIT_Y)){
			x = rand() % (LARGEUR - TAILLE_PAVES - 2) + 1;
			y = rand() % (HAUTEUR - TAILLE_PAVES - 2) + 1;
		}
